...
var tileApproaching : Vector2 # the tile that the creature is tweening towards to
...
func move(path):
	...
	for step in path:
		if (stepsCounter > 0):
			stepsCounter -= 1
#			TileMapAStar.astar.set_point_disabled(TileMapAStar.id_generate(TileMapAStar.world_to_map(step)), true)
			tileApproaching = step
			Tween.interpolate_property(self, "position", get_global_position(), step, 0.1, Tween.EASE_IN, Tween.EASE_OUT)
			Tween.start()
...
func activate():
	...
	for dir in directions:
		...
		if (Ray.get_collider() != null):
			if (Ray.get_collider().get_parent() == Player):
