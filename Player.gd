...
func move_to_position(direction):
	...
	# Move if path is clear and continue the game
	if (canMove == true):
		active = false
		Tween.interpolate_property(self, "position", get_global_position(), get_global_position() + direction, 0.1, Tween.EASE_IN, Tween.EASE_OUT)
		Tween.start()
		
		# Launch turns after the player's action (end turn)
		yield(get_tree().create_timer(0.2), "timeout")
		GameManager.calc_turn_order()
		GameManager.launch_round()
